%{?scl:%scl_package perltidy}

# HTML::Entities is optional
%if ! (0%{?rhel}) && ! (0%{?scl:1})
%bcond_without perltidy_enables_HTML_Entities
%else
%bcond_with perltidy_enables_HTML_Entities
%endif
...
%if %{with perltidy_enables_HTML_Entities}
BuildRequires:	%{?scl_prefix}perl(HTML::Entities)
%endif
BuildRequires:	%{?scl_prefix}perl(IO::File)
BuildRequires:	%{?scl_prefix}perl(Pod::Html)
...
%if %{with perltidy_enables_HTML_Entities}
Requires:	%{?scl_prefix}perl(HTML::Entities)
%endif
Requires:	%{?scl_prefix}perl(Pod::Html)
Provides:	%{?scl_prefix}perl-Perl-Tidy = %{version}-%{release}
